...
exports.sourceNodes = async ({ actions, createNodeId, getCache }, config) => {
  ...
  sabordata.forEach(async property => {
    const saborImageIds = await fetchImages(
      createNode,
      createNodeId,
      getCache,
      property
...
async function fetchImages(createNode, createNodeId, getCache, property) {
  console.log("fetch Image ids")
  imageIds = []
  var clientSettings = {
    loginUrl: "http://sabor-rets.connectmls.com/rets/server/login",
    username: process.env.RETS_SABOR_USER,

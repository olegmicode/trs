  ...
  componentWillMount() {
    if (typeof window !== "undefined" && window) {
      ...
      if (favorites !== null) {
        if (
          favorites.some(
            favorites => favorites.mlsid === this.props.property.mlsid
          )
        ) {
        ...
        if (
          ...
          this.setState({ fav: false }, () => {
            const newList = favorites.filter(
              item => item.mlsid !== property.mlsid
            )
  ...
  render() {
    return (
      <div
        ...
        onClick={() => {
          this.setFavorite(this.props.property)
        }}
      >
        {this.state.fav === true ? "Remove from Favorites" : "Add to Favorites"}
      </div>
      // <h2>test</h2>
    )
  }
